...
class PlanningGraph():
    ...
    def __init__(self, problem: Problem, state: str, serial_planning=True):
        ...
        self.action_node_set = self.get_a_nodes_set()
        self.s_levels = []
        self.a_levels = []
        self.create_graph()
        
        
    def get_a_nodes_set(self):
        nodes = set()
        for action in self.all_actions:
            nodes.add(PgNode_a(action))
        return nodes
    ...
    def add_action_level(self, level):
        ...
        def is_first_subset_of_second(first: list, second: list) -> bool:
            """
            Return true if the elements from the first list are present in the second
            """
            return len(set(first) - set(second)) == 0
       
        #all positive literals on literal level
        pos_literals = [node.symbol for node in self.s_levels[level] if node.is_pos]
        #all negative literals on literal level
        neg_literals = [node.symbol for node in self.s_levels[level] if not node.is_pos]
        
        level_actions = set()
        for action_node in self.action_node_set:
            # if positive and negative preconditions of given action are presents on current literal level
            if is_first_subset_of_second(action_node.action.precond_pos,pos_literals) and is_first_subset_of_second(action_node.action.precond_neg,neg_literals):
                #connect literal parent nodes with this node
                action_node.parents = action_node.prenodes
                
                # add action node as a child into the parent literal nodes
                for lnode in action_node.parents:
                    if not lnode.children:
                        lnode.children = set()
                    lnode.children.add(action_node)
                    
                level_actions.add(action_node)
        self.a_levels[level] = level_actions
    ...
    def add_literal_level(self, level):
        ...
        for anode in self.a_levels[level - 1]:
            self.s_levels[level].update(anode.effnodes)
            anode.children = anode.effnodes
            for eff_node in anode.effnodes:
                eff_node.parents.add(anode)
    ...
    def get_list_attr_set(self, node_a1: PgNode_a, node_a2: PgNode_a, attr_name):
        """
        Finds tuple of attribute value for 2 PgNode_a nodes given attribute name.
        Individual attribute values are wrapped or converted into set before being returned as 
        a tuple
        ...
        """
        return ( set(getattr(node_a1.action,attr_name)), set(getattr(node_a2.action,attr_name)) ) 
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # find all positive effects for node_a1 and node_a2
        eff_pos1, eff_pos2 = self.get_list_attr_set(node_a1, node_a2,"effect_add")
        # find all negative effects for node_a1 and node_a2
        eff_neg1, eff_neg2 = self.get_list_attr_set(node_a1, node_a2,"effect_rem")
        # return true if non-empty set of positive effects of node_a1 is subset of negative effects if node_a2
        # or if non-empty set of positive effects of node_a2 is subset of negative effects if node_a1
        #return (bool(eff_pos1) and  eff_pos1.issubset(eff_neg2)) or (bool(eff_pos2) and eff_pos2.issubset(eff_neg1))
        return (eff_pos1 & eff_neg2) or (eff_pos2 & eff_neg1)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # find all positive preconditions for node_a1 and node_a2
        pre_pos1, pre_pos2 = self.get_list_attr_set(node_a1, node_a2,"precond_pos")
        # find all negative effects for node_a1 and node_a2
        eff_neg1, eff_neg2 = self.get_list_attr_set(node_a1, node_a2,"effect_rem")
        # return true if some positive precondition of node_a1 are present in negative effects of node_a2       
        # or if some positive precondition of node_a2 are present in negative effects of node_a1
        return  ( pre_pos1 & eff_neg2) or ( pre_pos2 & eff_neg1 )
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # loop through node_a1 parent literal nodes
        for node1 in node_a1.parents:
            # loop though node_a2 parent literal nodes
            for node2 in node_a2.parents:
                # if at least one of literals in node_a1's parent is mutex 
                # to any of node_a2 parent return true, otherwise false 
                if node1.is_mutex(node2): return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # return true if nodes have the same symbol and opposite values of is_pos attribute
        #  false otherwise
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # loop through node_s1 parent action nodes
        for node1 in node_s1.parents:
            # loop through node_s2 parent action nodes
            for node2 in node_s2.parents:
                # if at least one of action nodes in node_s1's parent is mutex 
                # to any of node_s2 parent return true, otherwise false 
                if not node1.is_mutex(node2): return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        #dictionary mapping literal node from the goal to the level where it first appears
        #mapped values are initialized by -1
        goal_level_vals = {PgNode_s(a,True): -1 for a in self.problem.goal}
        #loop though all the levels
        for level_ind in range(len(self.s_levels)):
            # find all literals from goal that are present on current level
            present_nodes = set(goal_level_vals.keys()) & self.s_levels[level_ind]
            # loop through the list of goal literals from current level
            for goal in present_nodes:
                # if mapped value was not set before (on previous steps) assign 
                # it to the current level index
                if goal_level_vals[goal] == -1:
                    goal_level_vals[goal] = level_ind
        # sum all level indexes recorded in the dictionary            
        for val in list(goal_level_vals.values()):
            level_sum += val
